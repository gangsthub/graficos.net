<template>
  ...
  <form @submit.prevent="copy(email)">
    ...
    <button v-if="isSupported" type="submit" class="pill-button flex items-center gap-2">
      <Icon name="octicon:copy-24" class="inline" />
  ...
  <div class="w-full sm:w-2/3 md:w-1/2 lg-1/2">
    <a :href="`mailt&#111;&#58;${email}`" class="pill-button flex items-center gap-2 mb-4 no-underline">
      <Icon name="octicon:mail-24" class="inline" />
    ...
    <a
      ...
      target="_blank"
      class="pill-button flex items-center gap-2 mb-4 no-underline"
    >
...
const emailComp = ref<InstanceType<typeof BaseTextsEmail> | null>(null)
